...
function ResourcesPage({session}){
  ...
  // ── PDF Resources ──────────────────────────────────────────────────────────
  const PDF_BUCKET="tca-pdf-resources";
  const PKEY="tca_pdfs_v1";
  ...
  const [previewPdf,setPreviewPdf]=useState(null); // {id,title,url}

  useEffect(()=>{
    try{
      const local=localStorage.getItem(`pref_${PKEY}`);
      if(local)setPdfs(JSON.parse(local));
    }catch(e){}
    if(!session?.user?.id)return;
    (async()=>{
      try{
        const val=await getPref(session.user.id, PKEY);
        if(val)setPdfs(JSON.parse(val));
      }catch(e){}
    })();
  },[session]);
  ...
  const savePdfs=(list)=>{
    setPdfs(list);
    if(session?.user?.id){
      setPref(session.user.id, PKEY, JSON.stringify(list)).catch(()=>{});
    } else {
      try{localStorage.setItem(`pref_${PKEY}`,JSON.stringify(list));}catch(e){}
    }
  };
  ...
  const uploadPdf=async(file)=>{
    ...
    if(!isPdf){setPdfError("Please select a PDF file.");return;}
    if(file.size>MAX_PDF_BYTES){setPdfError("File is too large — max 25MB.");return;}
    if(!session?.user?.id){setPdfError("You must be signed in to upload a PDF.");return;}
    setUploadingPdf(true);
    try{
      const id=Date.now()+"_"+Math.random().toString(36).slice(2,8);
      const safeName=file.name.replace(/[^a-zA-Z0-9._-]/g,"_");
      const path=`${session.user.id}/${id}_${safeName}`;
      ...
      const newPdf={id,title:pdfTitle.trim()||file.name.replace(/\.pdf$/i,""),path,url:publicUrl,size:file.size,addedAt:new Date().toISOString()};
      savePdfs([...pdfs,newPdf]);
      setPdfTitle("");setShowAddPdf(false);
    }catch(e){
      setPdfError("Upload failed: "+(e.message||"please try again."));
    }finally{
      setUploadingPdf(false);
    }
  };

  const removePdf=async(p)=>{
    if(!confirm(`Delete "${p.title}"? This cannot be undone.`))return;
    try{await supabase.storage.from(PDF_BUCKET).remove([p.path]);}catch(e){}
    savePdfs(pdfs.filter(x=>x.id!==p.id));
    if(previewPdf?.id===p.id)setPreviewPdf(null);
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      {/* PDF Resources header */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{width:36,height:36,borderRadius:10,background:"rgba(240,90,126,0.15)",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <span style={{fontSize:18}}>📄</span>
          </div>
          <div>
            <div style={{fontSize:18,fontWeight:800,color:B.text}}>PDF Resources</div>
            <div style={{fontSize:12,color:B.textMuted}}>{pdfs.length} document{pdfs.length!==1?"s":""} saved</div>
          </div>
        </div>
        <button onClick={()=>{setShowAddPdf(p=>!p);setPdfError("");}} style={{padding:"8px 18px",borderRadius:9,border:"none",background:GL,color:"#0E0E10",cursor:"pointer",fontSize:12,fontWeight:800}}>
          {showAddPdf?"✕ Cancel":"+ Add PDF"}
        </button>
      </div>
      ...
      {showAddPdf&&(
        <div style={{padding:20,borderRadius:14,background:B.surface,border:`1px solid ${B.borderTeal}`}}>
          <div style={{fontSize:13,fontWeight:700,color:B.text,marginBottom:14}}>Add PDF Resource</div>
          <div style={{marginBottom:14}}>
            <label style={lS}>Title (optional)</label>
            <input value={pdfTitle} onChange={e=>setPdfTitle(e.target.value)} style={iS} placeholder="e.g. ICT Glossary, MES Playbook Cheat Sheet"/>
          </div>
          <label style={{display:"block",cursor:uploadingPdf?"default":"pointer"}}>
            <div style={{border:`2px dashed ${B.border}`,borderRadius:12,padding:"24px",textAlign:"center",background:"rgba(255,255,255,0.02)",transition:"all 0.2s"}}
              onDragOver={e=>{e.preventDefault();if(!uploadingPdf)e.currentTarget.style.borderColor=B.teal;}}
              onDragLeave={e=>{e.currentTarget.style.borderColor=B.border;}}
              onDrop={e=>{
                e.preventDefault();
                e.currentTarget.style.borderColor=B.border;
                ...
                if(file)uploadPdf(file);else setPdfError("Please drop a PDF file.");
              }}>
              <div style={{fontSize:24,marginBottom:8}}>📄</div>
              <div style={{fontSize:13,color:B.text,fontWeight:600}}>{uploadingPdf?"Uploading...":"Drop a PDF here"}</div>
              <div style={{fontSize:11,color:B.textMuted,marginTop:4}}>or click to browse — max 25MB</div>
            </div>
            <input type="file" accept=".pdf,application/pdf" disabled={uploadingPdf} style={{display:"none"}}
              onChange={e=>{const file=e.target.files[0];if(file)uploadPdf(file);e.target.value="";}}/>
          </label>
          {pdfError&&<div style={{marginTop:10,fontSize:12,color:B.loss}}>{pdfError}</div>}
        </div>
      )}
      ...
      {pdfs.length===0?(
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 20px",borderRadius:14,border:`2px dashed ${B.border}`,gap:14}}>
          <div style={{fontSize:48}}>📄</div>
          <div style={{fontSize:15,fontWeight:700,color:B.text}}>No PDFs yet</div>
          <div style={{fontSize:12,color:B.textMuted,textAlign:"center",lineHeight:1.6}}>
            Upload PDF guides, cheat sheets, or playbooks<br/>to build your personal document library
          </div>
          <button onClick={()=>setShowAddPdf(true)} style={{marginTop:4,padding:"10px 24px",borderRadius:9,border:"none",background:GL,color:"#0E0E10",cursor:"pointer",fontSize:13,fontWeight:800}}>+ Add Your First PDF</button>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {pdfs.map(p=>(
            <div key={p.id} style={{background:B.surface,border:`1px solid ${B.border}`,borderRadius:12,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:38,height:38,borderRadius:9,background:"rgba(240,90,126,0.12)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>📄</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,fontWeight:700,color:B.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.title}</div>
                <div style={{fontSize:11,color:B.textMuted}}>{fmtPdfSize(p.size)}{p.size?" · ":""}{new Date(p.addedAt).toLocaleDateString()}</div>
              </div>
              <div style={{display:"flex",gap:6,flexShrink:0}}>
                <button onClick={()=>setPreviewPdf(previewPdf?.id===p.id?null:p)} style={{padding:"6px 14px",borderRadius:7,border:`1px solid ${previewPdf?.id===p.id?B.teal:B.border}`,background:previewPdf?.id===p.id?`${B.teal}15`:"transparent",color:previewPdf?.id===p.id?B.teal:B.textMuted,cursor:"pointer",fontSize:11,fontWeight:700}}>{previewPdf?.id===p.id?"Hide":"👁 View"}</button>
                <a href={p.url} target="_blank" rel="noopener noreferrer" style={{padding:"6px 10px",borderRadius:7,border:`1px solid ${B.border}`,color:B.textMuted,fontSize:11,textDecoration:"none"}}>⤓</a>
                <button onClick={()=>removePdf(p)} style={{background:"none",border:`1px solid ${B.border}`,borderRadius:7,color:B.textMuted,cursor:"pointer",fontSize:11,padding:"6px 10px"}}>✕</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Inline PDF preview */}
      {previewPdf&&(
        <div style={{background:B.surface,border:`1px solid ${B.borderTeal}`,borderRadius:14,overflow:"hidden"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",borderBottom:`1px solid ${B.border}`}}>
            <div style={{fontSize:12,fontWeight:700,color:B.text}}>{previewPdf.title}</div>
            <button onClick={()=>setPreviewPdf(null)} style={{background:"none",border:"none",color:B.textMuted,cursor:"pointer",fontSize:13}}>✕ Close</button>
          </div>
          <iframe src={previewPdf.url} title={previewPdf.title} style={{width:"100%",height:700,border:"none",display:"block"}}/>
        </div>
      )}
    </div>
  );
...
function WeeklyReview({trades, session}){
  ...
  const downloadDebrief=()=>{
    if(!debrief)return;
    const weekLabel=`Week of ${selectedWeek} to ${weekEnd}`;
    const weekStats=`${weekTrades.length} trades | $${weekTrades.reduce((a,t)=>a+t.pnl,0).toFixed(2)} net P&L | ${weekTrades.length?Math.round(weekTrades.filter(t=>t.result==="Win").length/weekTrades.length*100):0}% WR`;

    // Convert markdown-style sections to HTML
    const bodyHtml=debrief.split("\n").map(line=>{
...
<head>
  <meta charset="utf-8"/>
  <title>TCA Weekly Debrief – ${weekLabel}</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    body{font-family:'Helvetica Neue',Arial,sans-serif;color:#1a1a2e;padding:48px;max-width:720px;margin:0 auto;font-size:14px;line-height:1.7;}
    .header{border-bottom:3px solid #8B5CF6;padding-bottom:20px;margin-bottom:28px;}
    .logo{font-size:11px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#8B5CF6;margin-bottom:8px;}
    h1{font-size:22px;font-weight:900;color:#1a1a2e;margin-bottom:6px;}
    .meta{font-size:12px;color:#666;margin-top:6px;}
    h2{font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#8B5CF6;margin:24px 0 8px;padding-bottom:4px;border-bottom:1px solid #e8e4f8;}
    ...
    @media print{body{padding:32px;}@page{margin:24px;}}
  </style>
</head>
<body>
  <div class="header">
    <div class="logo">The Candlestick Academy</div>
    <h1>Weekly Trading Debrief</h1>
    <div class="meta">${weekLabel} &nbsp;·&nbsp; ${weekStats}</div>
  </div>
  ${bodyHtml}
  <div class="footer">Generated by TCA Journal · ${new Date().toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}</div>
</body>
</html>`;
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:debrief?16:0}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              {debrief&&!debriefLoading&&(
                <button onClick={downloadDebrief} style={{padding:"8px 14px",borderRadius:9,border:`1px solid ${B.border}`,background:"rgba(255,255,255,0.04)",color:B.textMuted,cursor:"pointer",fontSize:12,fontWeight:700}}>⤓ PDF</button>
              )}
              <button
